...
if($bln==1) {$nama_bulan='JANUARI';}
			...
			$this->pdf->Ln(10);
			$this->pdf->Cell(0,10,'LAPORAN PENJUALAN UD ADITYA ',0,0,'C');
			$this->pdf->Ln(5);
		...
		function CreateReportDetail(){
			...
			$this->pdf->Cell(45,10,'Penjualan',1,0,'C',0);
			...
			$tahun=$_POST['tahun'];
			$bulan=$_POST['bulan'];
			$query=mysqli_query($db,"SELECT tgl, sum(total) as total from tb_penjualan
			 where year(tgl)=$tahun and month(tgl)=$bulan group by tgl");
			$hitungtotal=mysqli_query($db,"SELECT tgl, sum(total) as grandtotal from tb_penjualan
...
where year(tgl)=$tahun and month(tgl)=$bulan");
			...
			{
				$hitungpenjualan=$hitunggrandtotal['grandtotal'];
			...
			$this->pdf->Cell(45,10,'Rp '.$hitungpenjualan_format,1,0,'C',0);
