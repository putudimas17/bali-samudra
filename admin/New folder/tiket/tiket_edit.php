			...
			<div class="panel panel-default">
				<div class="panel-heading" align="center">Edit Data Tiket</div>
					<div class="panel-body">
						<form name="modal_popup" method="POST" action="tiket/tiket_edit.php">
							<div class="form-group" hidden="">
								<label id="kode_brg">Kode Tiket</label>
									<input type="text" id="kode_brg" placeholder="Kode Tiket" name ="kode_tkt" class="form-control" value="<?php echo $r['kode_tkt']; ?>" required readonly>
								</div>
							...
							<div class="form-group" >
								<label for="nama_brg">Nama tiket</label>
									<input type="text" id="nama_brg" placeholder="Nama tiket" name ="nama_brg" class="form-control" value="<?php echo $r['nama_brg']; ?>" required oninvalid="this.setCustomValidity('nama tiket tidak boleh kosong')" oninput="setCustomValidity('')">
							</div>
							<div class="form-group" >
		...
		$query=mysqli_query($db, "SELECT * FROM tb_barang WHERE id_kategori='$id_kategori' and nama_brg='$nama_brg' ");
			$cek = mysqli_num_rows( $query );
				if ( $cek > 0 ) {
					...
					echo "<script>window.location='../index.php?page=datatiket';</script>";
				}
