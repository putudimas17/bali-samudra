...
function readyInput($db){
	$sql = 'select * from tb_retur_pembelian where id_user = '.$_SESSION['karyawan'].' and status=0';
 	...
 	if($gg->num_rows > 0){
 		...
 		// get detail 
	    $sql = "select * from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg where i.id = '".$head[0]['id']."'";
	 	$gg = $db->query($sql);
 		$detail = array();
 		echo $db->error;
	 	if($gg->num_rows > 0){
...
if(isset($_GET['action'])){
	switch($_GET['action']){
		case 'view':
			$sql = " select l.INV as INV_pembelian,i.id, i.tgl, i.total, i.INV, k.Nama as pegawai, j.Nama as nama_suplier from tb_retur_pembelian i left join tb_supel j on i.id_supel = j.id left join tb_user k on i.id_user = k.id left join tb_pembelian l on i.no_pembelian = l.id_pembelian where i.id = '".$_POST['id']."' order by i.id desc";
		 	...
		 	if($gg->num_rows > 0){
			 	...
			 	$detail = [];
			 	$sql = 'select i.id_retur, i.id, i.id_barang, j.nama_brg,i.jumlah,i.harga,i.total from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where  id_retur = "'.$_POST['id'].'"';
			 	$gg = $db->query($sql);
		 		$detail = array();
			 	if($gg->num_rows > 0){
			 		while($row = mysqli_fetch_assoc($gg)) {
		...
		case 'fetch-barang-dari-pembelian':
			$sql = 'select i.kode_brg, i.id_kategori, i.nama_brg, i.stok, i.harga, i.satuan from tb_barang i left join tb_detail_pembelian j on i.kode_brg = j.id_barang where j.id_pembelian = '.$_POST['id_pembelian'].'';
		 	$gg = $db->query($sql);
	 		$barang = array();
	 		echo $db->error;
		 	if($gg->num_rows > 0){
		...
		case 'search-barang-dari-pembelian':
			$sql = 'select i.kode_brg, i.id_kategori, i.nama_brg, j.jumlah, j.harga, i.satuan from tb_barang i left join tb_detail_pembelian j on i.kode_brg = j.id_barang where i.kode_brg = "'.$_POST['kode_brg'].'" AND j.id_pembelian = '.$_POST['id_pembelian'].'';
		 	$gg = $db->query($sql);
	 		$barang = array();
		 	if($gg->num_rows > 0){
		 		while($row = mysqli_fetch_assoc($gg)) {
		...
		case 'total':
			$sql = "UPDATE tb_retur_pembelian set total = '".$_POST['total'] ."', status='1' WHERE id= '".$_POST['id']."'";
		 	if ($db->query($sql) === TRUE) {
		    	...
		    	$sql = "select * from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where  i.id_retur = '".$_POST['id']."'";
			 	$gg = $db->query($sql);
			 	if($gg->num_rows > 0){
			 		$toJSON = [
				 		'status' => 'success',
		...
		case 'save':
			/*
			$checkNoEMptyStock = "SELECT jumlah from tb_detail_pembelian i left join tb_retur_pembelian j on i.id_pembelian = j.no_pembelian where i.id_barang = '".$_POST['id_barang']."' AND  id_retur = ".$_POST['id_retur']."";
			...
			$sql = 'select * from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where i.id_barang = "'.$_POST['id_barang'].'" AND  id_retur = "'.$_POST['id_retur'].'"';
			$gg = $db->query($sql);
 			if($gg->num_rows > 0){
 				 try{
				    $sql = "UPDATE tb_detail_retur_pembelian set jumlah = ".$_POST['jumlah'].", total=".$_POST['jumlah']*$_POST['harga']." where id_barang = '".$_POST['id_barang']."' AND id_retur = '".$_POST['id_retur']."'";
					$updated = mysqli_query($db,$sql);
					echo $db->error;
					if ($updated) {
						$sql = 'select i.id_retur, i.id, i.id_barang, j.nama_brg,i.jumlah,i.harga,i.total from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where  id_retur = "'.$_POST['id_retur'].'"';
					 	$gg = $db->query($sql);
				 		$detail = array();
					 	if($gg->num_rows > 0){
					 		while($row = mysqli_fetch_assoc($gg)) {
 				...
 				$sql = "INSERT INTO  tb_detail_retur_pembelian (id_retur,id_barang,jumlah,harga,total) VALUES ('".$_POST['id_retur']."','".$_POST['id_barang']."',".$_POST['jumlah'].",".$_POST['harga'].",".$_POST['jumlah']*$_POST['harga'].")";
				if ($db->query($sql) === TRUE) {
					$sql = 'select i.id_retur,i.id,i.id_barang,j.nama_brg,i.jumlah,i.harga,i.total from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where  id_retur = "'.$_POST['id_retur'].'"';
				 	$gg = $db->query($sql);
			 		$detail = array();
		...
		break;
		case 'delete':
			$sql = "DELETE from tb_detail_retur_pembelian where id_barang = '".$_POST['kode_brg']."' AND id = '".$_POST['id']."'";
			$updated = mysqli_query($db,$sql);
			if ($updated) {
				$sql = 'select i.id_retur,i.id,i.id_barang,j.nama_brg,i.jumlah,i.harga,i.total from tb_detail_retur_pembelian i left join tb_barang j on i.id_barang = j.kode_brg  where  id_retur = "'.$_POST['id_retur'].'"';
			 	$gg = $db->query($sql);
		...
		break;
		case 'search-barang':
		 	$sql = 'select kode_brg ,id_kategori ,nama_brg ,stok ,j.harga,satuan from tb_barang  i left join tb_detail_pembelian j on i.kode_brg = j.id_barang where j.id_barang = "'.$_POST['kode_brg'].'" AND  j.id_pembelian = "'.$_POST['id_pembelian'].'"';
		 	$gg = $db->query($sql);
	 		$barang = array();
		 	if($gg->num_rows > 0){
		 		while($row = mysqli_fetch_assoc($gg)) {
